...
namespace Project.Scripts.Utils.Extensions {
    public static class ListExtensions {
        ...
        public static T PickRandom<T>(this IList<T> list,T exeption)
        {
            if (list.Count == 0) throw new InvalidOperationException("Cannot pick a random element from an empty list.");
            if (list.Count == 1 && list[0].Equals(exeption)) throw new InvalidOperationException("Cannot pick a random element from the list because the only element is the exception.");
            ...
            do
            {
                ...
                selected = list[index];
            } while (selected.Equals(exeption));
            return selected;
